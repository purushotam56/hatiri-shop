...
export default function SellerEditProductVariantsPage() {
  ...
  const fetchProductGroup = async () => {
    try {
      ...
      const groupData = await apiEndpoints.getSellerProductGroupDetail(
        String(orgId),
        groupIdValue,
        token || "",
      );
      ...
      const formattedVariants: VariantEdit[] = group.variants.map((v: ProductGroup) => {
        // Extract label from name and SKU
        const baseName = group.name;
        const label = (v.name || "").replace(baseName, "").trim();
        const skuSuffix = (v.sku || "").replace(
          group.variants[0].sku.replace(/[^-]*-?$/, ""),
          "",
        );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const invalidVariant = form.variants.find(
        (v) => !v.label || !v.skuSuffix || !v.price || !v.quantity,
      );
      ...
      const data = await apiEndpoints.updateProductVariants(
        groupId,
        formData,
        token,
      );
          ...
          <Card className="border-2 border-primary-200 dark:border-primary-800">
            ...
            <CardBody className="space-y-4">
              {form.variants.map((variant, index) => (
                <Card
                  key={variant.id}
                  className="bg-default-50 dark:bg-default-100/50"
                >
