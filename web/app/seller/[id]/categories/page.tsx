...
function CategoriesContent() {
  ...
  const fetchCategories = async () => {
    try {
      setLoading(true)
      setError('')
      const data = await apiEndpoints.getSellerCategories(organisationId!, token!)
  ...
  const handleCreateCategory = async () => {
    ...
    try {
      ...
      await apiEndpoints.createSellerCategory(organisationId!, formData, token!)
  ...
  const handleDeleteCategory = async (categoryId: number) => {
    ...
    try {
      ...
      await apiEndpoints.deleteSellerCategory(organisationId!, categoryId, token!)
      await fetchCategories()
    } catch (err: any) {
            ...
            {categories.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {categories.map((category) => (
                  <Card key={category.id} className="bg-white hover:shadow-lg transition">
