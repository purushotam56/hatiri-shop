...
export default function OrderDetailPage() {
  ...
  useEffect(() => {
    ...
    const fetchOrder = async () => {
      try {
        const token = localStorage.getItem("sellerToken");
        const data = await apiEndpoints.getSellerOrderDetail(
          String(orgId),
          String(orderId),
          token || "",
        );
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      ...
      const data = await apiEndpoints.updateSellerOrderStatus(
        String(orgId),
        String(orderId),
        selectedStatus,
        token,
          ...
          <>
            ...
            <Card>
              <CardHeader className="flex justify-between items-start">
                ...
                <span
                  className={`px-4 py-2 rounded-full text-sm font-semibold ${statusColors[order.status] || statusColors.pending}`}
                >
              ...
              <CardBody>
                <div className="space-y-3">
                  {order.items.map((item) => (
                    <div
                      key={item.id}
                      className="flex justify-between items-center p-3 bg-default-100 dark:bg-default-100/50 rounded-lg"
                    >
