...
export function ProductDetailClient({
  ...
  const [_addedMessage, _setAddedMessage] = useState("");
  ...
  const _handleAddToCart = (variantToAdd: Product) => {
    addToCart({
      id: variantToAdd.id || 0,
      productId: product?.id || variantToAdd.id,
      variantId: variantToAdd.id,
      name: variantToAdd.name || "Product",
      price: variantToAdd.price || 0,
      quantity: quantity,
      currency: variantToAdd.currency || "INR",
      sku: variantToAdd.sku,
      unit: variantToAdd.unit,
      bannerImage: variantToAdd.bannerImage,
      image: variantToAdd.image,
    });

    _setAddedMessage(`✓ Added to cart!`);
    setTimeout(() => {
      _setAddedMessage("");
      setQuantity(1);
    }, 1500);
    ...
    <div className="pb-28">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 p-4 max-w-7xl mx-auto">
        ...
        <div className="lg:col-span-3 space-y-3">
          ...
          <Card
            className="relative overflow-hidden group border border-divider/50"
            shadow="none"
          >
                ...
                {variants.map((variant) => {
                  ...
                  return (
                    <button
                      key={variant.id}
                      type="button"
                      className={`rounded-xl border-2 cursor-pointer transition-all duration-200 overflow-hidden shadow-sm hover:shadow-md text-left w-full ${
                        isSelected
                    ...
                    >
                      ...
                      <div className="p-2.5 space-y-1.5">
                        <div className="flex items-start justify-between gap-1">
                          <p className="font-bold text-xs text-foreground line-clamp-1 flex-1">
                            {variantName}
                          </p>
                          {isSelected && (
                        ...
                        <PriceDisplay
                          ...
                          price={variant.price}
                          priceVisibility={
                            product?.organisation?.priceVisibility
                          }
                        />
                        ...
                        {variant.stock > 0 ? (
                          <div className="flex items-center gap-1">
                            ...
                            <p className="text-xs font-semibold text-success">
                              {variant.stock} {variant.stockUnit} left
                            </p>
                          </div>
                        ) : (
                          <div className="flex items-center gap-1">
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                <div
                  className={`flex flex-col items-center gap-1 px-3 py-2 rounded-lg ${
                    product.stock > 0 ? "bg-success/20" : "bg-danger/20"
                  }`}
                >
                  <div
