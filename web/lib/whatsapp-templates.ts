 ...
 */
export const generateWhatsAppWebURL = (phoneNumber: string, message: string): string => {
  const cleanNumber = phoneNumber.replace(/[^\d+]/g, '')
  const encodedMessage = encodeWhatsAppMessage(message)
...
export const getQuickProductMessage = (product: Product): string => {
  return `Hi! I'm interested in "${product.name}" - ₹${parseFloat(String(product.price)).toFixed(0)}${product.sku ? ` (SKU: ${product.sku})` : ''}`
}
...
export const buildProductLink = (
  ...
  const webDomain = domain || process.env.NEXT_PUBLIC_WEB_DOMAIN || 'localhost:3000'
...
export const generateDetailedInquiryWithLink = ({
  ...
  domain,
}: ProductInquiry & { organisationUniqueCode: string; domain?: string }): string => {
  const link = buildProductLink(organisationUniqueCode, product.id, domain)
  const lines = [
    `Hi${sellerName ? ` ${sellerName}` : ''}! 👋`,
    '',
