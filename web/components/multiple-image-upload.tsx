    ...
    <div className="space-y-3">
      ...
      {value.length < maxImages && (
        <div
          ...
          `}
        >
          <svg className="w-10 h-10 text-default-400 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
          </svg>
          <p className="text-sm text-default-600">Add Images</p>
          <p className="text-xs text-default-400 mt-1">{value.length}/{maxImages}</p>
        </div>
      )}
      ...
      <p className="text-xs text-default-400">
        First image will be the main product image. You can reorder images by using up/down arrows.
      </p>
    </div>
  )
}
