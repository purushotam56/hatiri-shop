...
import { COLOR_PALETTES, type ColorPalette } from "@/config/theme-config";
...
export function RuntimePaletteSelector() {
  ...
  const applyPalette = (paletteKey: string) => {
    ...
    root.style.setProperty("--theme-success", extractHSL(palette.colors.success));
    root.style.setProperty("--theme-warning", extractHSL(palette.colors.warning));
    ...
    console.log(`🎨 PALETTE CHANGED TO: ${palette.name.toUpperCase()}`);
    console.log(`  Primary: hsl(${primaryValue})`);
    console.log(`  Secondary: hsl(${secondaryValue})`);
    console.log(`  Update Key: ${updateKey + 1}`);
    console.log('  ✅ CSS variables updated - components should update immediately!');
  };
      ...
      <CardHeader>
        <div className="space-y-2">
          ...
          <p className="text-sm text-default-500">
            Click any palette to switch colors instantly. Changes are applied in real-time!
          </p>
          <div className="flex items-start gap-2 p-3 bg-primary-50 dark:bg-primary-900/20 rounded-lg border border-primary-200 dark:border-primary-800">
            ...
            <div className="text-xs text-primary-800 dark:text-primary-200">
              <strong>Production Note:</strong> For permanent palette changes, update <code className="bg-primary-100 dark:bg-primary-900 px-1 rounded">DEFAULT_PALETTE</code> in <code className="bg-primary-100 dark:bg-primary-900 px-1 rounded">config/theme-config.ts</code>
            </div>
          </div>
        </div>
      </CardHeader>
        ...
        {selectedPalette !== "ocean" && (
          <div className="flex items-center justify-center p-4 bg-success-50 dark:bg-success-900/20 rounded-lg border border-success-200 dark:border-success-800">
            ...
            <p className="text-sm text-success-800 dark:text-success-200">
              Palette changed to <strong>{COLOR_PALETTES[selectedPalette]?.name}</strong> - Colors updated!
            </p>
          </div>
        )}
      </CardBody>
